...
class FileSystemOperations:
    ...
    def copy_file(self, source: Path, target: Path, preserve_metadata: bool = True) -> bool:
        """
        ...
        try:
            ...
            self.logger.error("Unexpected error copying file %s → %s: %s", source, target, e)
            return False
    
    def copy_directory(self, source: Path, target: Path, 
        ...
        try:
            ...
            self.logger.error("Unexpected error copying directory %s → %s: %s", source, target, e)
            return False
    ...
    def list_files(self, directory: Path, pattern: str = "*", 
        ...
        try:
            ...
            self.logger.debug("Found %d files matching '%s' in %s", 
                            len(file_paths), pattern, directory.name)
            return file_paths
    ...
    def list_directories(self, directory: Path, pattern: str = "*") -> List[Path]:
        ...
        try:
            ...
            self.logger.debug("Found %d directories matching '%s' in %s", 
                            len(directories), pattern, directory.name)
            return directories
    ...
    def ensure_directory_exists(self, directory: Path, 
        ...
        try:
            ...
            self.logger.error("Unexpected error creating directory %s: %s", directory, e)
            return False
    ...
    def remove_directory(self, directory: Path, recursive: bool = False) -> bool:
        ...
        try:
            ...
            self.logger.error("Unexpected error removing directory %s: %s", directory, e)
            return False
