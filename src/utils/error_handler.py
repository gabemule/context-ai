...
from utils.exceptions import ConfigurationError, StorageError, ValidationError
...
def validate_file_path(
    path: str, must_exist: bool = True, must_be_dir: bool = False
) -> None:
    """
...
def validate_embedding_name(name: str) -> None:
    ...
    if len(name) > MAX_EMBEDDING_NAME_LENGTH:
        raise ValidationError(
            f"Embedding name too long (max {MAX_EMBEDDING_NAME_LENGTH} characters)"
        )
