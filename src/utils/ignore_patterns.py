...
class IgnorePatternMatcher:
    ...
    def should_ignore(self, file_path: Path) -> bool:
        ...
        try:
            ...
            self.logger.warning("Error checking ignore pattern for %s: %s", file_path, e)
            return False
    ...
    def _match_pattern(self, pattern: str, path_str: str, file_path: Path) -> bool:
        ...
        if file_path.is_dir() and pattern.endswith('/**'):
            ...
            if fnmatch.fnmatch(path_str, dir_pattern) or path_str.endswith('/' + dir_pattern):
                return True
...
class IgnoreFileResolver:
    ...
    def resolve_ignore_patterns(self, custom_ignore_file: Optional[Path] = None) -> IgnorePatternMatcher:
        """
        ...
        try:
            ...
            self.logger.debug("Loaded %d ignore patterns from %s", len(patterns), source)
            ...
            return IgnorePatternMatcher(list(DEFAULT_IGNORE_PATTERNS), self.project_path)
    ...
    def _load_ignore_file(self, ignore_file: Path) -> List[str]:
        ...
        try:
            ...
            self.logger.debug("Loaded %d patterns from %s", len(patterns), ignore_file.name)
            return patterns
...
class FileFilter:
        ...
        try:
            ...
            try:
                ...
                if file_size > MAX_FILE_SIZE_BYTES:
                    return False, f"file too large (>{MAX_FILE_SIZE_BYTES // (1024*1024)}MB)"
    ...
    def filter_files(self, file_paths: List[Path]) -> Tuple[List[Path], dict]:
        ...
        self.logger.info("Filtered %d files: %d accepted, %d ignored, %d unsupported", 
                        stats['total'], stats['accepted'], stats['ignored'], stats['unsupported'])
        
        return accepted, stats


def create_file_filter(project_path: Path, custom_ignore_file: Optional[Path] = None) -> FileFilter:
    """
