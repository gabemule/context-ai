...
class EmbeddingProgressTracker:
    ...
    def start_embedding_generation(self, files: List[Path], embedding_name: str) -> None:
        """
    ...
    def _show_initial_stats(self, embedding_name: str, files: List[Path]) -> None:
        ...
        stats_table.add_row("Total Size", f"{self.stats.bytes_total / BYTES_PER_MB:.1f} MB")
        ...
        ext_info = ", ".join([f"{ext}: {count}" for ext, count in sorted(extensions.items())])
    ...
    def _show_completion_stats(self) -> None:
        ...
        completion_table.add_row("Data Processed", f"{self.stats.bytes_processed / BYTES_PER_MB:.1f} MB")
        completion_table.add_row("Time Elapsed", f"{self.stats.elapsed_time:.1f}s")
        completion_table.add_row("Processing Rate", f"{self.stats.processing_rate:.1f} files/sec")
        ...
        # Success message
        if self.stats.errors_count == 0:
            self.console.print(f"🎉 [bold green]Successfully created {self.stats.chunks_created} chunks from {self.stats.files_processed} files![/bold green]")
        else:
            self.console.print(f"⚠️  [yellow]Completed with {self.stats.errors_count} errors. Created {self.stats.chunks_created} chunks from {self.stats.files_processed - self.stats.errors_count} files.[/yellow]")
...
def create_embedding_progress_tracker(show_details: bool = True) -> EmbeddingProgressTracker:
    """
