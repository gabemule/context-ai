    ...
    def load_yaml(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not isinstance(data, dict):
                self.logger.error(
                    "Invalid YAML structure in %s: expected dict, got %s",
                    file_path,
                    type(data).__name__,
                )
                return None
    ...
    def validate_config_data(self, data: Dict[str, Any], config_file: Path) -> bool:
        ...
        for field in required_fields:
            if field not in data:
                self.logger.error(
                    "Missing required field '%s' in %s", field, config_file
                )
                return False
        ...
        for feature_name, feature_value in features.items():
            if feature_name not in supported_features:
                self.logger.warning(
                    "Unknown feature '%s' in %s", feature_name, config_file
                )

            if not isinstance(feature_value, bool):
                self.logger.error(
                    "Feature '%s' must be boolean in %s", feature_name, config_file
                )
                return False
    ...
    def validate_required_files(self, mode_path: Path) -> bool:
        ...
        for required_file in PROMPT_REQUIRED_FILES:
            ...
            if not file_path.exists():
                self.logger.error(
                    "Required file '%s' not found in %s", required_file, mode_path
                )
                return False
            ...
            try:
                if file_path.stat().st_size == 0:
                    self.logger.error(
                        "Required file '%s' is empty in %s", required_file, mode_path
                    )
                    return False
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    if not content.strip():
                        self.logger.error(
                            "Required file '%s' contains only whitespace in %s",
                            required_file,
                            mode_path,
                        )
                        return False
                ...
                self.logger.error(
                    "Cannot read required file '%s' in %s: %s",
                    required_file,
                    mode_path,
                    e,
                )
...
class PromptConfigLoader:
    ...
    def load_mode_config(self, mode_path: Path) -> Optional[PromptModeConfig]:
        ...
        self.logger.debug(
            "✅ Validated and loaded config for mode: %s v%s",
            config.name,
            config.version,
        )
        return config
...
class PromptFileLoader:
    ...
    def load_file(self, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            self.logger.debug(
                "Loaded file: %s (%d chars)", file_path.name, len(content)
            )
            return content
...
class PromptBuilder:
    ...
    def _ensure_user_prompt_config(self):
        ...
        if not default_dir.exists():
            ...
            raise RuntimeError(
                f"Cannot initialize prompt system: default templates not found at {default_dir}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"Cannot initialize prompt system: failed to copy defaults - {e}"
            )
                ...
                if not user_file.exists() or user_file.stat().st_size == 0:
                    ...
                    if default_mode_dir.exists():
                        ...
                        if default_file.exists():
                            ...
                            self.logger.error(
                                "Cannot repair %s: default not found", filename
                            )
                            return False
                    else:
                        self.logger.error(
                            "Cannot repair mode %s: default mode not found", mode_name
                        )
                        return False
    ...
    def _repair_missing_file(self, target_file: Path, source_file: Path):
        ...
        try:
            if not source_file.exists():
                self.logger.error(
                    "Cannot repair %s: source file not found at %s",
                    target_file,
                    source_file,
                )
                return
    ...
    def build_prompt(self, question: str, context: Optional[str] = None) -> str:
        ...
        if not mode_config:
            ...
            if not available_modes:
                ...
                raise RuntimeError(
                    "No valid prompt modes found. Configuration may be corrupted."
                )
            ...
            if "standard" in available_modes:
                ...
                final_prompt = self._build_file_based_prompt(
                    question, context, "standard", self._get_mode_config("standard")
                )

                # 🆕 NOVO: Log prompt data for fallback mode
                self._log_prompt_if_available(
                    final_prompt,
                    question,
                    context,
                    "standard",
                    self._get_mode_config("standard"),
                )
                return final_prompt
            ...
            self.logger.error(error_msg)
            raise ValueError(
                f"Invalid prompt mode '{prompt_mode}'. Available: {', '.join(available_modes)}"
            )

        # Build prompt using file-based system
        final_prompt = self._build_file_based_prompt(
            question, context, prompt_mode, mode_config
        )

        # 🆕 NOVO: Log prompt data
        self._log_prompt_if_available(
            final_prompt, question, context, prompt_mode, mode_config
        )
    ...
    def _build_file_based_prompt(
        self, question: str, context: str, mode: str, config: PromptModeConfig
    ) -> str:
        ...
        global_instructions = self.file_loader.load_global_file(
            self._prompt_base_dir, "global_instructions.md"
        )
        ...
        if config.features.get("security", False):
            security_instructions = self.file_loader.load_global_file(
                self._prompt_base_dir, "security_instructions.md"
            )
        ...
        mode_dir = self._prompt_base_dir / mode
        core_instructions = self.file_loader.load_mode_file(
            mode_dir, "core_instructions.md"
        )
        ...
        if config.features.get(
            "cross_analysis", False
        ) and self._is_cross_project_context(context):
            cross_analysis = self.file_loader.load_mode_file(
                mode_dir, "cross_analysis.md"
            )
            if cross_analysis:
                prompt_parts.append(
                    f"\n<cross_analysis>\n{cross_analysis}\n</cross_analysis>"
        ...
        if config.features.get("guidelines", False):
            ...
            if detected_languages:
                ...
                try:
                    ...
                    guidelines_content = (
                        guidelines_manager.get_guidelines_for_languages(
                            detected_languages
                        )
                    )
                    ...
                    if guidelines_content:
                        ...
                        self.logger.debug(
                            f"Applied guidelines for languages: {', '.join(detected_languages)}"
                        )
                        self._last_guidelines_content = guidelines_content
                    else:
                        self.logger.debug(
                            f"No guidelines found for languages: {', '.join(detected_languages)}"
                        )
...
{question}
        ...
        final_instructions = self.file_loader.load_mode_file(
            mode_dir, "final_instructions.md"
        )
    ...
    def _is_cross_project_context(self, context: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"XML parsing failed for cross-project detection, using fallback: {e}"
            )
            # Fallback to old method if XML parsing fails
            return (
                "Cross-Project Analysis" in context
                and "Project Correlations" in context
            )
    ...
    def _extract_languages_from_xml_context(self, context: str) -> List[str]:
        ...
        try:
            ...
            if tech_stack_elem is not None and tech_stack_elem.text:
                ...
                self.logger.debug(
                    f"🎯 XML parsing: found {len(languages)} languages, {len(filtered_languages)} with guidelines"
                )
    ...
    def _filter_programming_languages(self, languages: List[str]) -> List[str]:
        ...
        try:
            ...
            if len(filtered) != len(languages):
                ...
                self.logger.debug(
                    f"🚫 Filtered out languages without guidelines: {filtered_out}"
                )
    ...
    def _extract_languages_fallback(self, context: str) -> List[str]:
        ...
        if match:
            ...
            self.logger.debug(
                f"📝 Fallback regex: found {len(languages)} languages, {len(filtered_languages)} with guidelines"
            )
            return filtered_languages
    ...
    def discover_available_modes(self) -> Dict[str, PromptModeConfig]:
        ...
        if not self._prompt_base_dir.exists():
            self.logger.warning(
                "Prompt config directory not found: %s", self._prompt_base_dir
            )
        ...
        for item in self._prompt_base_dir.iterdir():
            ...
            if not config:
                self.logger.warning(
                    "⚠️ Mode '%s' ignored: invalid or missing mode.yaml", mode_name
                )
                continue
            ...
            self.logger.debug(
                "✅ Mode '%s' loaded: %s v%s", mode_name, config.name, config.version
            )

        self.logger.info(
            "🎯 Discovered %d enabled modes: %s",
    ...
    def get_available_modes(self) -> List[str]:
        ...
        return (
            ...
            if discovered
            else ["minimal", "standard", "comprehensive", "strict"]
        )
    ...
    def _log_prompt_if_available(
        ...
        try:
            ...
            global_instructions = (
                self.file_loader.load_global_file(
                    self._prompt_base_dir, "global_instructions.md"
                )
                or ""
            )
            ...
            core_instructions = (
                self.file_loader.load_mode_file(mode_dir, "core_instructions.md") or ""
            )
            cross_analysis = (
                self.file_loader.load_mode_file(mode_dir, "cross_analysis.md")
                if config.features.get("cross_analysis", False)
                and self._is_cross_project_context(context)
                else ""
            )
            final_instructions = (
                self.file_loader.load_mode_file(mode_dir, "final_instructions.md") or ""
            )
