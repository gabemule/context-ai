...
class MultiEmbeddingResultMerger:
    ...
    def merge_multi_embedding_results(
        ...
        if not (len(documents) == len(metadatas) == len(distances)):
            self.logger.error(
                "Mismatched result lengths: docs=%d, meta=%d, dist=%d",
                len(documents),
                len(metadatas),
                len(distances),
            )
        ...
        for i, (doc, metadata, distance) in enumerate(
            ...
            grouped[embedding_name].append(
                {"text": doc, "distance": distance, "metadata": metadata, "index": i}
            )
    ...
    def format_results_for_display(
        self, results: List[QueryResult], max_results: int = 10
    ) -> str:
        """
...
"""
    ...
    def get_cross_embedding_stats(self, results: List[QueryResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_embedding": (
                max(
                    embedding_stats.keys(),
                    key=lambda k: embedding_stats[k]["avg_score"],
                )
                if embedding_stats
                else None
            ),
