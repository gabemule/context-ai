...
class VectorStoreManager:
            ...
            try:
                ...
                try:
                    ...
                    self._collection = client.create_collection(
                        ...
                        metadata={
                            "description": "Context-AI embeddings with "
    ...
    def store_embeddings(
        ...
        if not documents or not embeddings or not metadatas:
            raise ConfigurationError(
                "Documents, embeddings, and metadatas cannot be empty"
            )
        ...
        try:
            ...
            document_ids = [
                f"{embedding_name}_{uuid.uuid4().hex[:8]}_{i}"
            ...
            self.logger.info(
                "✅ Stored %d documents for embedding '%s'",
                len(documents),
                embedding_name,
            )
            return True
    ...
    def query_embeddings(
        ...
        try:
            ...
            results = collection.query(
                query_texts=query_texts, where=where_clause, n_results=n_results
            )
    ...
    def get_embedding_info(self, embedding_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            results = collection.get(
                where={METADATA_EMBEDDING_NAME: embedding_name}, include=["metadatas"]
            )
            ...
            self.logger.error(
                "Failed to get embedding info for '%s': %s", embedding_name, e
            )
            return None
    ...
    def delete_embedding(self, embedding_name: str) -> bool:
        ...
        try:
            ...
            if not results["ids"]:
                self.logger.warning(
                    "No documents found for embedding '%s'", embedding_name
                )
                return False

            # Delete all documents for this embedding
            collection.delete(where={METADATA_EMBEDDING_NAME: embedding_name})

            self.logger.info(
                "✅ Deleted %d documents for embedding '%s'",
                len(results["ids"]),
                embedding_name,
            )
            return True
