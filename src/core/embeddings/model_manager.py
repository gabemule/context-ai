...
class ModelInfo:
    ...
    def __init__(self, name: str, dimensions: int, max_seq_length: int, 
                 size_mb: float, description: str = ""):
        self.name = name
...
class EmbeddingModelManager:
    ...
    def get_model_info(self, model_name: str) -> ModelInfo:
        ...
        if model_name not in AVAILABLE_MODELS:
            ...
            raise ConfigurationError(f"Model '{model_name}' not available. Available models: {available}")
    ...
    def load_model(self, model_name: str = None, show_progress: bool = True) -> SentenceTransformer:
        """
        ...
        try:
            ...
            if not self.is_model_cached(model_name):
                ...
                self.logger.info("📥 Downloading model (this may take a few minutes)...")
            
            # Load model (will download if not cached)
            model = SentenceTransformer(model_name, cache_folder=str(self._model_cache_dir))
    ...
    def generate_embeddings(self, texts: List[str], model_name: str = None, 
                          batch_size: int = None, show_progress: bool = True) -> List[List[float]]:
        """
        ...
        for model_name in AVAILABLE_MODELS:
            ...
            if cache_path.exists():
                try:
                    ...
                    self.logger.warning("Error calculating cache size for %s: %s", model_name, e)
    ...
    def _show_download_info(self, model_info: ModelInfo) -> None:
        ...
        table.add_row("Cache Location", str(self._get_model_cache_path(model_info.name)))
        
        console.print(Panel(table, expand=False))
        console.print("⏳ This is a one-time download. Future uses will be much faster!")
        console.print()
...
def create_embeddings(texts: List[str], model_name: str = None, 
                     batch_size: int = None, show_progress: bool = True) -> List[List[float]]:
    """
