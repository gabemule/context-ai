...
class LangChainChunker(ChunkerProtocol):
    ...
    def chunk_text(self, text: str, file_path: str) -> List[TextChunk]:
        ...
        try:
            ...
            for i, chunk_text in enumerate(chunks):
                ...
                metadata.update({
                    ChunkingMetadata.CHUNK_TYPE: self._detect_chunk_type(chunk_text, language),
                    ChunkingMetadata.TOKEN_COUNT: self._estimate_token_count(chunk_text),
                    ChunkingMetadata.FILE_SIZE: len(text)
                })
                
                text_chunks.append(TextChunk(text=chunk_text, metadata=metadata))
            
            self.logger.debug("Chunked %s into %d chunks (language: %s)", 
                            Path(file_path).name, len(text_chunks), language or "unknown")
    ...
    def _get_splitter_for_language(self, language: Optional[str]) -> RecursiveCharacterTextSplitter:
        """
        ...
        try:
            ...
            self.logger.warning("Error creating language-specific splitter: %s", e)
    ...
    def _detect_chunk_type(self, chunk_text: str, language: Optional[str]) -> str:
        ...
        if chunk_lower.startswith('class '):
            ...
            return 'interface'
        elif chunk_lower.startswith('#') or chunk_lower.startswith('//') or chunk_lower.startswith('/*'):
            return 'comment'
...
def create_langchain_chunker(strategy: Optional[ChunkingStrategy] = None) -> LangChainChunker:
    """
