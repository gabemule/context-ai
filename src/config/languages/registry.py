...
from .loader import ConfigurationError, load_languages_config
from .models import ChunkingPriority, LanguagesConfig, ResolvedLanguageConfig
...
class LanguagesRegistry:
    ...
    def _load_config(self, force_reload: bool = False) -> LanguagesConfig:
        ...
        if self._config is None or force_reload or not self._cache_valid:
            ...
            if not self.languages_file.exists():
                self.logger.debug(
                    "Languages config missing, ensuring via SetupManager..."
                )
            ...
            try:
                self.logger.debug(
                    f"Loading languages configuration from: {self.languages_file}"
                )
    ...
    def _get_resolved_languages(
        self, force_reload: bool = False
    ) -> Dict[str, ResolvedLanguageConfig]:
        ...
        if self._resolved_languages is None or force_reload:
            ...
            try:
                ...
                self.logger.debug(
                    f"Resolved {len(self._resolved_languages)} language configurations"
                )
    ...
    def get_language_separators(self, language: str) -> List[str]:
        ...
        if language not in resolved:
            self.logger.warning(
                f"Language '{language}' not found, using default separators"
            )
            return ["\n\n", "\n", " ", ""]
    ...
    def reset_language_to_default(self, language: str) -> bool:
        ...
        try:
            ...
            with open(self.languages_file, "w", encoding="utf-8") as f:
                ...
                config_dict = {
                    "languages": {
                        name: lang.dict(exclude_unset=True)
                        for name, lang in current_config.languages.items()
                    },
