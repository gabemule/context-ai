...
class LanguageValidator:
    ...
    def validate_priority(priority: str) -> str:
        ...
        if not ChunkingPriority.is_valid(normalized):
            raise ValueError(f"chunking_priority must be one of: {', '.join(ChunkingPriority.ALL)}")
        return normalized
...
class ConfigMerger:
    ...
    @staticmethod
    def merge_extensions(base_extensions: List[str], current_extensions: List[str]) -> List[str]:
    ...
    @staticmethod
    def merge_configs(base_config: 'LanguageConfig', current_config: 'LanguageConfig') -> Dict:
...
class InheritanceResolver:
    ...
    def resolve_language(self, name: str, visited: Optional[Set[str]] = None) -> 'ResolvedLanguageConfig':
        ...
        if config.extends:
            ...
            merged_data = ConfigMerger.merge_configs(base_config.to_language_config(), config)
...
class LanguageConfig(BaseModel):
    ...
    separators: Optional[List[str]] = Field(None, description="Text separators for chunking")
    chunking_priority: str = Field(ChunkingPriority.MEDIUM, description="Priority for chunking")
    extends: Optional[str] = Field(None, description="Language to inherit configuration from")
    additional_separators: Optional[List[str]] = Field(None, description="Additional separators to merge")
...
class ResolvedLanguageConfig(BaseModel):
    ...
    def from_language_config(cls, config: LanguageConfig) -> 'ResolvedLanguageConfig':
        ...
        if config.separators is None:
            raise ValueError(f"Language '{config.name}' has no separators and no inheritance")
...
class GlobalSettings(BaseModel):
    ...
    default_chunking_priority: str = Field(ChunkingPriority.LOW, description="Default priority for new languages")
    auto_detect_languages: bool = Field(True, description="Enable automatic language detection")
    cache_language_configs: bool = Field(True, description="Cache loaded configurations")
...
class LanguagesConfig(BaseModel):
    ...
    languages: Dict[str, LanguageConfig] = Field(default_factory=dict, description="Language configurations")
