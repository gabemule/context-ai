...
from .models import LanguagesConfig
...
class ConfigurationCopier(ABC):
    ...
    def copy_configuration_files(self, target_directory: Path) -> CopyResult:
        ...
        try:
            ...
            result = CopyResult(
                files_copied=files_copied, files_skipped=files_skipped, errors=errors
            )
            ...
            return CopyResult(files_copied=0, files_skipped=0, errors=[error_msg])
...
class YAMLConfigLoader:
    ...
    def load(self, file_path: Path) -> Dict[str, Any]:
        ...
        if config_data is None:
            raise ConfigurationError(
                f"Failed to load YAML configuration", file_path=file_path
            )

        if not isinstance(config_data, dict):
            raise ConfigurationError(
                f"Configuration must be a YAML dictionary", file_path=file_path
            )
...
class DefaultConfigGenerator:
    ...
    def copy_default_config(cls, target_path: Path) -> None:
        ...
        try:
            ...
            logger.info(
                f"Copied default configuration from {template_path} to {target_path}"
            )

        except Exception as e:
            raise ConfigurationError(
                f"Failed to copy default configuration: {str(e)}",
...
def load_languages_config(file_path: Path) -> LanguagesConfig:
    ...
    try:
        ...
        raise ConfigurationError(
            f"Invalid configuration structure: {str(e)}", file_path=file_path, cause=e
        )
