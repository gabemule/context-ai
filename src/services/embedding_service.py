...
from config.constants import DEFAULT_QUERY_RESULTS, DEFAULT_DISPLAY_RESULTS
from utils.logging import get_logger
from utils.error_handler import validate_file_path, validate_embedding_name
...
class EmbeddingService:
    ...
    def generate_embedding(self, path: str, name: str, ignore_file: str = None, show_progress: bool = True) -> Dict[str, Any]:
        """
        ...
        if ignore_file:
            ...
            self.logger.info("📋 Using intelligent ignore file detection (.contextignore → .gitignore → defaults)")
        ...
        try:
            ...
            if all_documents:
                self.logger.info("🔄 Generating embeddings for %d chunks...", len(all_documents))
        ...
        self.logger.info("✅ Embedding generation completed: %d chunks from %d files", 
    ...
    def select_embeddings(self) -> List[str]:
        ...
        try:
            ...
            raise ValidationError("inquirer not installed. Install with: pip install inquirer")
...
class QueryService:
    ...
    def query_context(self, 
        ...
        if not active.selected:
            raise ValidationError("No active embeddings selected. Use 'context-ai select' first")
        ...
        if verbose:
            ...
            if processed_query.expanded_terms:
                self.logger.info("📝 Expanded terms: %s", ', '.join(processed_query.expanded_terms))
        ...
        try:
            ...
            normalized_results = self.result_merger.merge_multi_embedding_results(raw_results)
            
            if not normalized_results:
                return "No relevant context found."
            
            # Format results using the new context formatter
            display_results = max_results if max_results is not None else DEFAULT_DISPLAY_RESULTS
            ...
            if verbose:
                ...
                stats_info = f"\n## Context Statistics\n"
                stats_info += f"- Token count: {stats['token_count']} ({stats['token_percentage']:.1f}% of limit)\n"
                stats_info += f"- Chunks: {stats['chunk_count']} from {stats['source_count']} sources\n"
                stats_info += f"- Sources: {', '.join(stats['sources'])}\n"
                if stats['truncated']:
                    stats_info += f"- ⚠️ Context was truncated due to token limit\n"
...
class AIService:
    ...
    def ask_question(self, question: str) -> str:
        ...
        if not config.ai or config.active_provider not in config.ai:
            raise ValidationError("AI provider not configured. Use 'context-ai config set --claude-key YOUR_KEY'")
