...
class ContextManager:
    ...
    def _fetch_fresh_context(
        self, question: str, max_tokens: int, verbose: bool
    ) -> str:
...
class DisplayManager:
    ...
    def display_token_stats(
        ...
        stats_text.append(
            f"Total: {total_used:,} tokens ({total_pct:.1f}%)\n", style="bold"
        )
        stats_text.append(f"Input: {actual_input:,} • Output: {actual_output:,}\n")

        if include_history and chat_context_tokens > 0:
            stats_text.append(
                f"Breakdown: {code_context_tokens//1000}K code + {chat_context_tokens//1000}K history\n"
            )
        else:
        ...
        stats_text.append(
            f"Remaining: {remaining:,} tokens ({100-total_pct:.1f}%)", style="dim"
        )

        panel = Panel(
            stats_text, title="📊 Token Usage", border_style="cyan", padding=(0, 1)
        )
        console.print()
        console.print(panel)
...
class APIManager:
    ...
    def ask_with_progress(
        ...
        try:
            ...
            response = self._make_api_call(
                question, context, max_tokens, verbose, live_streaming_callback
            )
            ...
            if not VSCODE_MODE:
                try:
                    ...
                    final_content = (
                        Markdown(accumulated_text.strip())
            ...
            if verbose:
                ...
                self.logger.info(
                    "⏱️ %s API (streaming): %.1f seconds (%s in, %s out)",
                    provider_name,
                    elapsed,
                    f"{input_tokens:,}",
                    f"{output_tokens:,}",
    ...
    def _make_api_call(
        self, question: str, context: str, max_tokens: int, verbose: bool, text_callback
    ):
...
class ChatCommandHandler:
    ...
    def handle_command(
        self, command: str, verbose: bool
    ) -> tuple[bool, bool]:  # (handled, new_verbose)
        ...
        if command == "/embeddings":
            ...
            self.logger.info(
                "🔧 Verbose mode %s", "enabled" if new_verbose else "disabled"
            )
    ...
    def _show_history(self):
        ...
        if stats["turns"] > 0:
            self.logger.info(
                "💬 Chat History: %d turns, %s total tokens, avg %d tokens/turn",
                stats["turns"],
...
class AIService:
    ...
    def __init__(self):
        ...
        if not self.provider_config or not self.provider_config.api_key:
            ...
            raise ConfigurationError(
                f"{provider_name} API key not configured. Set it with: context-ai config set --{self.active_provider}-key YOUR_KEY"
            )
        ...
        self.chat_handler = ChatCommandHandler(
            self.settings_manager, self.chat_history, self.logger
        )
    ...
    def ask_question(
        ...
        try:
            ...
            if context_format in ["json", "xml", "plain", "markdown"]:
                return self._handle_non_ai_format(
                    question, context_format, verbose, output_file, copy_to_clipboard
                )
            ...
            contexts = self._get_contexts(
                question, token_allocation, verbose, include_history
            )
            ...
            self._log_context_usage(
                token_allocation, contexts, input_tokens, include_history
            )

            # Save context to session
            self._save_context_to_session(
                contexts["combined"], question, token_allocation
            )

            # Get AI response
            response, was_streamed = self.ai_api.ask_with_progress(
                question, contexts["combined"], max_tokens, verbose, include_history
            )
            ...
            if include_history:
                self._add_to_chat_history(
                    question, response, contexts["combined"], input_tokens, verbose
                )

            # Handle output
            self._handle_output(
                response.content, output_file, copy_to_clipboard, was_streamed
            )

            # Display stats
            self.display_manager.display_token_stats(
                response,
                input_tokens,
                contexts["total_tokens"],
                token_allocation["question_tokens"],
                verbose,
                include_history,
                contexts["code_tokens"],
                contexts["chat_tokens"],
            )
    ...
    def start_chat(self, verbose: bool = False) -> None:
        ...
        self.logger.info(
            "💬 Starting chat session with %s%s...", provider_name, mode_text
        )
    ...
    def _get_contexts(
        ...
        # Get code context
        code_context = self.context_manager.get_context(
            question, token_allocation["code_context_tokens"], verbose, include_history
        )
        ...
        if chat_context:
            ...
            total_tokens = (
                chat_context_tokens
                + separator_tokens
                + token_allocation["code_context_tokens"]
            )
        else:
    ...
    def _log_context_usage(
        ...
        window_usage_pct = (
            (contexts["total_tokens"] / token_allocation["total_context_tokens"]) * 100
            if token_allocation["total_context_tokens"] > 0
            else 0
        )

        self.logger.info(
            "📊 Context Window: %dK used (%.1f%% of %dK limit)",
            contexts["total_tokens"] // 1000,
            window_usage_pct,
            token_allocation["total_context_tokens"] // 1000,
        )

        if include_history and contexts["chat_tokens"] > 0:
            self.logger.info(
                "📊 Breakdown: %dK code + %dK history (%d turns) = %dK total",
                contexts["code_tokens"] // 1000,
                contexts["chat_tokens"] // 1000,
                len(self.chat_history.history),
                contexts["total_tokens"] // 1000,
            )
        else:
        ...
        provider_name = self.active_provider.title()
        self.logger.info(
            "📊 Input: %sK tokens → %s", input_tokens // 1000, provider_name
        )
        ...
        if input_tokens > large_threshold:
            self.logger.warning(
                "⚠️  Large context (%s tokens) - %s may be slow",
                f"{input_tokens:,}",
                provider_name,
            )
            ...
            self.logger.info(
                "✅ Small context (%s tokens) - should be fast", f"{input_tokens:,}"
            )

    def _save_context_to_session(
        self, context: str, question: str, token_allocation: Dict[str, int]
    ) -> None:
    ...
    def _update_session_with_response(
        self, response, input_tokens: int, include_history: bool
    ) -> None:
        ...
        if session:
            ...
            output_tokens = response.usage.get("output_tokens", 0)
            total_tokens = (
                response.usage.get("input_tokens", input_tokens) + output_tokens
            )
            ...
            if session.is_chat:
                if session.session_data["turns"]:
                    ...
                    session.session_data["session_totals"][
        ...
        try:
            while True:
                try:
                    ...
                    handled, verbose = self.chat_handler.handle_command(
                        question, verbose
                    )
                    if handled:
                        continue
    ...
    def _handle_chat_exit(self) -> None:
        ...
        if stats["turns"] > 0:
            self.logger.info(
                "💬 Chat completed: %d turns, %s total tokens",
                stats["turns"],
                f"{stats['total_tokens']:,}",
            )
        else:
