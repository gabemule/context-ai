...
class ChatHistoryManager:
    ...
    def add_turn(self, question: str, response: str, context: str = "", tokens: int = 0) -> None:
    ...
    def get_summary_prompt(self) -> str:
        ...
        return f"""Please summarize this conversation history in 2-3 sentences, focusing on:
...
class AIService:
    ...
    def ask_question(self, question: str, context_format: str = "ai_friendly", 
                     ...
                     output_file: str = None, include_history: bool = False) -> str:
        """
        ...
        try:
            ...
            from config.constants import (
                ...
                CLAUDE_MIN_RESPONSE_TOKENS, CLAUDE_MAX_RESPONSE_TOKENS, CHAT_HISTORY_TOKEN_RATIO
            )
            ...
            if include_history:
                ...
                if verbose and chat_context:
                    self.logger.info("💬 Chat history: %d tokens (%d turns)", 
                                   chat_context_tokens, len(self.chat_history.history))
            else:
            ...
            if verbose:
                self.logger.info("📊 Context limit: %d tokens (65%% of %d)", total_context_tokens, CLAUDE_MAX_TOKENS)
                if include_history:
                    self.logger.info("📊 Allocation: %d code + %d history = %d total context", 
                                   code_context_tokens, chat_context_tokens, context_tokens)
                ...
                if input_tokens > 100000:
                    self.logger.warning("⚠️  Large context detected (%s tokens) - Claude may be slow", f"{input_tokens:,}")
            ...
            response = self._ask_claude_with_progress(question, context, max_tokens, verbose, include_history)
            ...
            if include_history:
                total_tokens_used = response.usage.get("input_tokens", input_tokens) + response.usage.get("output_tokens", 0)
                self.chat_history.add_turn(question, response.content, context, total_tokens_used)
            ...
            while True:
                try:
                    ...
                    if question.lower() in ['exit', 'quit', 'bye']:
                        ...
                        if stats["turns"] > 0:
                            self.logger.info("📊 Session stats: %d turns, %s total tokens", 
                                           stats["turns"], f"{stats['total_tokens']:,}")
                    ...
                    if question.lower() == '/history':
                        stats = self.chat_history.get_stats()
                        if stats["turns"] > 0:
                            self.logger.info("💬 Chat History: %d turns, %s total tokens, avg %d tokens/turn",
                                           stats["turns"], f"{stats['total_tokens']:,}", stats["avg_tokens_per_turn"])
                        else:
                        ...
                        verbose = not verbose
                        self.logger.info("🔧 Verbose mode %s", "enabled" if verbose else "disabled")
                        continue
    ...
    def _ask_claude_with_progress(self, question: str, context: str, max_tokens: int, 
                                 verbose: bool = False, include_history: bool = False):
        ...
        if verbose:
            ...
            progress_msg = f"🧠 Asking Claude ({context_info}, max {max_tokens:,} tokens)..."
        else:
            if include_history and hasattr(self, 'chat_history') and self.chat_history.history:
                progress_msg = f"🧠 Thinking... (with {len(self.chat_history.history)} turn context)"
            else:
        ...
        with console.status(progress_msg, spinner=spinner_type) as status:
            ...
            if response_tokens > 0:
                completion_msg = f"✅ Response received in {elapsed:.1f}s ({response_tokens:,} tokens)"
            else:
        ...
        if verbose:
            ...
            self.logger.info("⏱️ Claude API: %.1f seconds (%s in, %s out)", 
                           elapsed, f"{input_tokens:,}", f"{output_tokens:,}")
        
        return response
    
    def _get_cached_context(self, question: str, max_tokens: int, verbose: bool, include_history: bool) -> str:
        """Get context with intelligent caching for performance."""
        import hashlib
        import time
        from config.constants import ENABLE_CONTEXT_CACHE, CONTEXT_CACHE_TTL
