...
def _initialize_services(logger) -> ServiceContainer:
    ...
    return ServiceContainer(
        embedding_service=None, console=console  # Will be lazy loaded
    )
...
def _execute_generate_request(
    request: GenerateRequest, services: ServiceContainer, logger
) -> None:
...
def _auto_select_new_embedding(embedding_name: str, logger) -> None:
    ...
    try:
        ...
        if embedding_name not in current_selected:
            ...
            logger.info(
                "🎯 Auto-selected embedding '%s' for immediate use", embedding_name
            )
...
def add_generate_parser(subparsers) -> argparse.ArgumentParser:
    ...
    # Required arguments
    parser.add_argument("path", help=GENERATE_HELP["arguments"]["path"])

    parser.add_argument(
        "--name", "-n", required=True, help=GENERATE_HELP["arguments"]["name"]
    )

    # Optional arguments
    parser.add_argument(
        "--ignore-file", "-i", help=GENERATE_HELP["arguments"]["ignore_file"]
    )
