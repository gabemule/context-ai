...
def chat_session_context(request: ChatRequest, logger):
    ...
    session_args = {"verbose": request.verbose, "prompt_mode": request.prompt_mode}
...
def _create_chat_request(args: argparse.Namespace) -> ChatRequest:
    ...
    return ChatRequest(
        verbose=args.verbose, prompt_mode=getattr(args, "prompt_mode", None)
    )
    ...
    try:
        ...
        try:
            ...
            if not current_active.selected:
                ...
                logger.info(
                    "💡 Chat will work without context. Use 'context-ai select' to add context"
                )
            else:
                logger.info(
                    "📋 Active embeddings: %s", ", ".join(current_active.selected)
                )
        except Exception:
...
def _execute_chat_session(
    request: ChatRequest, services: ServiceContainer, logger
) -> None:
...
def add_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    # Optional arguments
    parser.add_argument(
        "--verbose", "-v", action="store_true", help=CHAT_HELP["arguments"]["verbose"]
    )
