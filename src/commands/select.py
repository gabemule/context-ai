...
def selection_context(request: SelectRequest, logger):
    ...
    if request.verbose:
        logger.info(
            "📋 Selection mode: %s",
            "Interactive" if request.is_interactive else "Direct",
        )
...
def _create_select_request(args: argparse.Namespace) -> SelectRequest:
    ...
    return SelectRequest(
        embeddings=embeddings, is_interactive=is_interactive, verbose=args.verbose
    )
...
def _get_current_selection_info(settings_manager, logger, verbose: bool) -> None:
    ...
    try:
        ...
        if current_selected:
            logger.info(
                "📋 Currently active embeddings: %s", ", ".join(current_selected)
            )
        else:
...
def _execute_direct_selection(
    request: SelectRequest, services: ServiceContainer, logger
) -> None:
    ...
    try:
        ...
        if request.verbose:
            _get_current_selection_info(
                services.settings_manager, logger, request.verbose
            )
...
def _execute_interactive_selection(
    request: SelectRequest, services: ServiceContainer, logger
) -> None:
...
def add_select_parser(subparsers) -> argparse.ArgumentParser:
    ...
    # Optional arguments
    parser.add_argument(
        "embeddings", nargs="*", help=SELECT_HELP["arguments"]["embeddings"]
    )

    parser.add_argument(
        "--verbose", "-v", action="store_true", help=SELECT_HELP["arguments"]["verbose"]
    )
