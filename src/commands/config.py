...
def add_config_parser(subparsers) -> argparse.ArgumentParser:
    ...
    config_set.add_argument(
        "--claude-key", help=CONFIG_SET_HELP["arguments"]["claude_key"]
    )
    ...
    config_set.add_argument(
        ...
        "-v",
        action="store_true",
        help=CONFIG_SET_HELP["arguments"]["verbose"],
    )
    ...
    config_list.add_argument(
        ...
        "-v",
        action="store_true",
        help=CONFIG_LIST_HELP["arguments"]["verbose"],
    )
    ...
    config_test.add_argument(
        ...
        "-v",
        action="store_true",
        help=CONFIG_TEST_HELP["arguments"]["verbose"],
    )
    ...
    config_validate.add_argument(
        ...
        "-v",
        action="store_true",
        help=CONFIG_VALIDATE_HELP["arguments"]["verbose"],
    )
    ...
    config_models.add_argument(
        "--provider", help=CONFIG_MODELS_HELP["arguments"]["provider"]
    )
    config_models.add_argument(
        "--verbose",
        "-v",
        action="store_true",
        help=CONFIG_MODELS_HELP["arguments"]["verbose"],
    )
    ...
    guidelines_show.add_argument(
        "language", help=GUIDELINES_SUBCOMMANDS_HELP["show"]["arguments"]["language"]
    )
    ...
    guidelines_edit.add_argument(
        "language", help=GUIDELINES_SUBCOMMANDS_HELP["edit"]["arguments"]["language"]
    )
    ...
    guidelines_reset.add_argument(
        "language", help=GUIDELINES_SUBCOMMANDS_HELP["reset"]["arguments"]["language"]
    )
    ...
    languages_subparsers = config_languages.add_subparsers(
        dest="languages_action", help="Languages actions"
    )
    ...
    languages_show.add_argument(
        "language", help=LANGUAGES_SUBCOMMANDS_HELP["show"]["arguments"]["language"]
    )
    ...
    languages_reset.add_argument(
        "language", help=LANGUAGES_SUBCOMMANDS_HELP["reset"]["arguments"]["language"]
    )
...
def _handle_config_test(args, settings_manager, logger) -> int:
    ...
    if not provider_config or not provider_config.api_key:
        ...
        logger.info(
            "Set your API key with: context-ai config set --%s-key YOUR_KEY",
            active_provider,
        )
        return 1
    ...
    try:
        if active_provider == "claude":
            ...
            client = get_claude_client(
                provider_config.api_key, provider_config.default_model
            )
            if client.validate_connection():
                logger.info(
                    "✅ %s API key is valid and working", active_provider.title()
                )
                return 0
            else:
        ...
        #     ...
            ...
            logger.error(
                "❌ API testing not implemented for provider: %s", active_provider
            )
            return 1
...
def _handle_config_models(args, settings_manager, logger) -> int:
    ...
    get_provider_function("get_available_providers")
    get_provider_function("get_available_models")
...
def _validate_active_provider_config(config, issues: List[str], logger) -> None:
    ...
    if active_provider == "claude":
        if not provider_config.api_key.startswith("sk-ant-"):
            issues.append(
                "❌ Claude API key format invalid (should start with 'sk-ant-')"
            )
            return
...
def _display_ai_provider(config, logger) -> None:
    ...
    if provider_config:
        ...
        logger.info(
            "   Model: %s (%dK tokens)", provider_config.default_model, max_tokens_k
        )
        logger.info(
            "   API Key: %s",
            "***configured***" if provider_config.api_key else "❌ not set",
        )
    else:
        logger.info(
            "   ❌ %s not configured - run: context-ai config set --%s-key YOUR_KEY",
...
def _display_embeddings(config, settings_manager, logger, verbose: bool) -> None:
    ...
    logger.info(
        "   Storage: %d max, cleanup after %d days",
        storage_config.max_embeddings,
        storage_config.cleanup_after_days,
    )
...
def _display_file_types(extensions: List[str], logger) -> None:
    ...
    try:
        ...
        for lang_name, lang_config in sorted(all_languages.items()):
            lang_extensions = [
                ext for ext in lang_config.extensions if ext in extensions
            ]
            if lang_extensions:
                logger.info(
                    "     %s: %s", lang_name.title(), ", ".join(sorted(lang_extensions))
                )
...
def _guidelines_edit(language: str, guidelines_manager, logger) -> int:
    ...
    try:
        ...
        logger.info(
            "✅ Guidelines will be reloaded automatically when you ask questions"
        )
        return 0
...
def _languages_list(languages_manager, logger) -> int:
    ...
    for name, config in sorted_languages:
        ...
        logger.info(
            ...
            extensions_str,
            inheritance_info,
            config.chunking_priority,
            guidelines_status,
        )

    logger.info("")
    logger.info(
        "📊 Total: %d languages, %d extensions",
        len(all_languages),
        len(languages_manager.get_supported_extensions()),
    )
...
def _languages_show(language: str, languages_manager, logger) -> int:
    ...
    if hasattr(config, "extends") and config.extends:
        ...
        logger.info(
            "Separators: %d total (%d inherited + %d additional)",
            len(config.separators),
            base_separators,
            additional,
        )
    else:
    ...
    if language in guidelines_languages:
        ...
        logger.info(
            "Guidelines: 📝 not available (create with: context-ai config guidelines edit %s)",
            language,
        )
...
def _languages_path(languages_manager, logger) -> int:
    ...
    if guidelines_missing > 0:
        ...
        logger.info("")
        logger.info(
            "💡 Create guidelines with: context-ai config guidelines edit <language>"
        )
