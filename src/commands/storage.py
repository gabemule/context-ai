...
def add_storage_parser(subparsers) -> argparse.ArgumentParser:
    ...
    storage_subparsers = parser.add_subparsers(dest="storage_action", help="Storage actions")
    ...
    info_parser.add_argument("--verbose", "-v", action="store_true", help=STORAGE_INFO_HELP["arguments"]["verbose"])
    ...
    clean_parser.add_argument("component", choices=["embeddings", "logs", "models", "temp", "all"], help=STORAGE_CLEAN_HELP["arguments"]["component"])
    clean_parser.add_argument("--older-than", type=int, help=STORAGE_CLEAN_HELP["arguments"]["older_than"])
    clean_parser.add_argument("--unused", action="store_true", help=STORAGE_CLEAN_HELP["arguments"]["unused"])
    clean_parser.add_argument("--all", action="store_true", help=STORAGE_CLEAN_HELP["arguments"]["all"])
    clean_parser.add_argument("--confirm", action="store_true", help=STORAGE_CLEAN_HELP["arguments"]["confirm"])
    clean_parser.add_argument("--verbose", "-v", action="store_true", help=STORAGE_CLEAN_HELP["arguments"]["verbose"])
    ...
    reset_parser.add_argument("--confirm", action="store_true", help=STORAGE_RESET_HELP["arguments"]["confirm"])
    reset_parser.add_argument("--verbose", "-v", action="store_true", help=STORAGE_RESET_HELP["arguments"]["verbose"])
    ...
    delete_parser.add_argument("embedding_name", help=STORAGE_DELETE_HELP["arguments"]["embedding_name"])
    delete_parser.add_argument("--verbose", "-v", action="store_true", help=STORAGE_DELETE_HELP["arguments"]["verbose"])
    ...
    optimize_parser.add_argument("--dry-run", action="store_true", help=STORAGE_OPTIMIZE_HELP["arguments"]["dry_run"])
    optimize_parser.add_argument("--aggressive", action="store_true", help=STORAGE_OPTIMIZE_HELP["arguments"]["aggressive"])
    optimize_parser.add_argument("--verbose", "-v", action="store_true", help=STORAGE_OPTIMIZE_HELP["arguments"]["verbose"])
...
def _display_storage_overview(storage_info: Dict[str, Any], logger) -> None:
    ...
    logger.info("   Total Size: %.2f MB (%.1f%% of %d MB limit)", total_mb, usage_pct, storage_limit)
...
def _display_embeddings_analysis(storage_info: Dict[str, Any], logger, verbose: bool = False) -> None:
...
def _display_active_embeddings(embeddings: List[str], logger) -> None:
    ...
    if active_list:
        logger.info("   Active: %s (%d selected)", ", ".join(active_list), len(active_list))
        inactive = [emb for emb in embeddings if emb not in active_list]
        if inactive:
            logger.info("   Inactive: %s (%d unused)", ", ".join(inactive), len(inactive))
    else:
...
def _display_embeddings_storage(storage_info: Dict[str, Any], logger) -> None:
    ...
    if embeddings_size > 0:
        ...
        logger.info("   Storage: %.2f MB (%.1f%% of total)", embeddings_mb, pct_of_total)
...
def _display_embeddings_details(embeddings: List[str], storage_info: Dict[str, Any], logger) -> None:
...
def _display_models_cache(storage_info: Dict[str, Any], logger, verbose: bool = False) -> None:
...
def _display_logs_analysis(storage_info: Dict[str, Any], logger, verbose: bool = False) -> None:
    ...
    if logs_size > 0:
        ...
        if types and any(count > 0 for count in types.values()):
            type_summary = ", ".join([f"{count} {log_type}" for log_type, count in types.items() if count > 0])
...
def _analyze_embeddings_recommendations(storage_info: Dict[str, Any], recommendations: List[str]) -> None:
    ...
    try:
        ...
        if len(inactive) > 0:
            inactive_potential_savings = len(inactive) * 15  # Estimate 15MB per embedding
            recommendations.append(f"Clean unused embeddings (save ~{inactive_potential_savings}MB)")
    except Exception:
        pass


def _analyze_logs_recommendations(storage_info: Dict[str, Any], recommendations: List[str]) -> None:
...
def _analyze_temp_recommendations(storage_info: Dict[str, Any], recommendations: List[str]) -> None:
...
def _analyze_models_recommendations(storage_info: Dict[str, Any], recommendations: List[str]) -> None:
...
def _clean_embeddings(args, storage_manager, logger) -> int:
    ...
    if older_than:
        ...
        logger.info("✅ Age-based embedding cleanup complete (%d embeddings cleaned)", cleaned_count)
    elif args.all:
        logger.info("🧹 Cleaning all embeddings data...")
        cleaned_count = storage_manager.clean_all_embeddings()
        logger.info("✅ Embeddings cleanup complete (%d embeddings cleaned)", cleaned_count)
    else:
...
def _clean_models(args, storage_manager, logger) -> int:
    ...
    if args.unused:
        ...
        logger.info("✅ Unused model cleanup complete (%d models cleaned)", cleaned_count)
    elif args.all:
        logger.info("🧹 Cleaning all model cache...")
        cleaned_count = storage_manager.clean_all_models()
        logger.info("✅ Model cache cleanup complete (%d models cleaned)", cleaned_count)
    else:
